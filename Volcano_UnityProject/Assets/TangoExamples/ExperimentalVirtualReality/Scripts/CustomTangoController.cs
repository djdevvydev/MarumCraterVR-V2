...
public class CustomTangoController : MonoBehaviour , ITangoPose
{
    ...
    void UpdateUsingInterpolatedPose(double t) {
        ...
        //if we aren't moving along the path
        if(GetComponent<CameraFollowPath>().cameraMoveAlongPath == false)
        {
    ...
    void ComputeTransformUsingPose(out Vector3 position, out Quaternion rot, TangoPoseData pose) {
        if(GetComponent<CameraFollowPath>().cameraMoveAlongPath == false)
        {
            position = new Vector3((float)pose.translation[0] * movementSpeedMultiplier,
                               (float)pose.translation[2],
                               (float)pose.translation[1] * movementSpeedMultiplier);
        }
        else
        {
